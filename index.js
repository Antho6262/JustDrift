...
const REGLEMENT_CHANNEL_ID = "1485618410086731787";
const PILOTE_ROLE_ID = "1474163725064863868";
...
// Envoyer le message de validation du règlement
async function sendReglementMessage(channel) {
    const embed = new EmbedBuilder()
        .setColor(0xFF4500)
        .setTitle("📋 Validation du Règlement JustDrift")
        .setDescription(
            "🏎️ **Tu as lu et accepté le règlement de JustDrift ?**\n\n" +
            "En cliquant sur le bouton ci-dessous, tu confirmes avoir pris connaissance des règles du serveur et tu t'engages à les respecter sur la piste comme en dehors.\n\n" +
            "Rejoins la communauté de pilotes qui partagent la même passion du drift — respect, fair-play et bonne ambiance sont les valeurs de JustDrift ! 🔥\n\n" +
            "*Bienvenue dans la famille JustDrift — que la fumée de pneus soit avec toi !* 💨"
        )
        .setFooter({ text: "JustDrift • Lis bien avant de valider !" })
        .setTimestamp();

    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
            .setCustomId("valider_reglement")
...
client.once("ready", async () => {
    ...
    // ── Message règlement au démarrage ──
    const reglementChannel = client.channels.cache.get(REGLEMENT_CHANNEL_ID);
    if (reglementChannel) {
        reglementChannel.messages.fetch({ limit: 10 }).then(async messages => {
            const exists = messages.some(msg => msg.author.id === client.user.id && msg.components.length > 0);
            if (!exists) {
                await sendReglementMessage(reglementChannel);
...
client.on("interactionCreate", async interaction => {
    try {
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId === "valider_reglement") {
                const piloteRole = interaction.guild.roles.cache.get(PILOTE_ROLE_ID);
                if (!piloteRole) return await interaction.reply({ content: "❌ Rôle introuvable, contacte un staff.", ephemeral: true });

                if (interaction.member.roles.cache.has(PILOTE_ROLE_ID)) {
                    return await interaction.reply({ content: "✅ Tu as déjà le rôle **Pilote** !", ephemeral: true });
                }

                try {
                    await interaction.member.roles.add(piloteRole);
                    await interaction.reply({
                        content: "🏎️ Bienvenue dans la famille **JustDrift** ! Tu as désormais le rôle **Pilote** et accès à tous les salons du serveur. Bonne route et que le drift soit avec toi ! 🔥",
                        ephemeral: true
                    });
                    console.log(`✅ Rôle Pilote donné à ${interaction.user.username}`);
                } catch (e) {
                    console.error("Erreur rôle pilote:", e);
                    await interaction.reply({ content: "❌ Erreur lors de l'attribution du rôle, contacte un staff.", ephemeral: true }).catch(() => {});
                }
                return;
            }
