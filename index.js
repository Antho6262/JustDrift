...
// Helper réponse safe — ne crash jamais
...
client.on("guildMemberAdd", async member => {
    ...
    if (welcomeChannel) {
        ...
        } catch (e) { console.error("Erreur message bienvenue:", e); }
    }
});
...
// Intercepter toutes les erreurs non gérées pour éviter les crashs
process.on("unhandledRejection", (error) => {
    console.error("Erreur non gérée (ignorée):", error?.message || error);
});
...
client.on("interactionCreate", async interaction => {
    try {
        if (interaction.isCommand() && interaction.commandName === "clean") {
            ...
            if (amount < 1 || amount > 100) return await interaction.reply({ content: "❌ Nombre entre 1 et 100.", ephemeral: true });
            try {
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId.startsWith("modal_accept_duel_")) {
                ...
                db.get("SELECT * FROM duels WHERE id = ? AND status = 'waiting'", [duelId], async (err, duel) => {
                    try {
                        if (!duel) return await safeReply(interaction, { content: "❌ Ce duel n'existe plus.", ephemeral: true });
                        if (interaction.user.id !== duel.challenged_id) return await safeReply(interaction, { content: "❌ Ce duel ne te concerne pas.", ephemeral: true });
                        ...
                        await safeReply(interaction, { content: `✅ Duel accepté ! Rendez-vous sur **${duel.map_name}** !`, ephemeral: true });
                    } catch (e) { console.error("Erreur modal duel:", e); }
                });
            }
            return;
        }
        ...
        if (interaction.isButton()) {
            if (interaction.customId.startsWith("accept_duel_")) {
                ...
                db.get("SELECT * FROM duels WHERE id = ?", [duelId], async (err, duel) => {
                    try {
                        if (!duel) return await safeReply(interaction, { content: "❌ Duel introuvable.", ephemeral: true });
                        if (interaction.user.id !== duel.challenged_id) return await safeReply(interaction, { content: "❌ Ce duel ne te concerne pas.", ephemeral: true });
                    ...
                    } catch (e) { console.error("Erreur accept_duel:", e); }
                });
                return;
            }
            ...
            if (interaction.customId.startsWith("refuse_duel_")) {
                ...
                db.get("SELECT * FROM duels WHERE id = ?", [duelId], async (err, duel) => {
                    try {
                        ...
                        await safeReply(interaction, { content: "✅ Duel refusé.", ephemeral: true });
                    } catch (e) { console.error("Erreur refuse_duel:", e); }
                });
                return;
            }
            ...
            if (interaction.customId.startsWith("duel_done_")) {
                ...
                db.get("SELECT * FROM duels WHERE id = ? AND status = 'ongoing'", [duelId], async (err, duel) => {
                    try {
                        if (!duel) return await safeReply(interaction, { content: "❌ Duel introuvable ou déjà terminé.", ephemeral: true });

                        const isChallenger = interaction.user.id === duel.challenger_id;
                        const isChallenged = interaction.user.id === duel.challenged_id;
                        if (!isChallenger && !isChallenged) return await safeReply(interaction, { content: "❌ Tu ne participes pas à ce duel.", ephemeral: true });
                        if (isChallenger && duel.challenger_done) return await safeReply(interaction, { content: "⚠️ Tu as déjà indiqué que tu avais fini !", ephemeral: true });
                        if (isChallenged && duel.challenged_done) return await safeReply(interaction, { content: "⚠️ Tu as déjà indiqué que tu avais fini !", ephemeral: true });
                        ...
                        await safeReply(interaction, { content: "✅ Noté ! En attente de l'adversaire...", ephemeral: true });
                    ...
                    } catch (e) { console.error("Erreur duel_done:", e); }
                });
                return;
            }
            ...
            if (interaction.customId === "open_ticket") {
                ...
                } catch (e) { console.error("Erreur open_ticket:", e); }
                return;
            }
            ...
            if (interaction.customId.startsWith("close_ticket_")) {
                try {
                    ...
                    if (!ticketChannel) return await safeReply(interaction, { content: "❌ Salon introuvable", ephemeral: true });
                ...
                } catch (e) { console.error("Erreur close_ticket:", e); }
                return;
            }
            ...
            if (interaction.customId.startsWith("vote_button_")) {
                ...
                } catch (e) { console.error("Erreur vote_button:", e); }
                return;
            }

            // Notation voitures/maps — ignorer tout ce qui n'est pas rate_ ou ratemap_
        ...
        if (interaction.isStringSelectMenu()) {
            if (interaction.customId.startsWith("select_vote_")) {
                ...
                db.get("SELECT * FROM votes_sondage WHERE message_id = ? AND user_id = ?", [messageId, userId], async (err, row) => {
                    try {
                        if (row && !interaction.member.roles.cache.has(staffRoleId)) return await interaction.editReply({ content: "⚠️ Tu as déjà voté !" }).catch(() => {});
                        db.run(
                    ...
                    } catch (e) { console.error("Erreur select_vote:", e); }
                });
                return;
            }
            ...
            try {
                ...
                db.get("SELECT * FROM votes WHERE vehicle_id = ? AND user_id = ?", [vehicleId, userId], async (err, row) => {
                    if (row && !interaction.member.roles.cache.has(staffRoleId)) return await interaction.editReply({ content: "⚠️ Tu as déjà noté ce véhicule/parcours !" }).catch(() => {});
            ...
            } catch (e) { console.error("Erreur notation select:", e); }
        }
    } catch (e) {
        console.error("Erreur interaction générale:", e?.message || e);
    }
});
