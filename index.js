...
// Helper pour répondre sans doublon
async function safeReply(interaction, options) {
    try {
        ...
        console.error("Erreur safeReply:", err);
    }
}
...
// Récupérer le meilleur temps d'un joueur sur une map
...
// Formater un temps en ms → mm:ss.ms
...
// Récupérer les infos du serveur FiveM
...
// Mettre à jour le message de statut serveur
...
client.on("interactionCreate", async interaction => {
    if (interaction.isCommand() && interaction.commandName === "clean") {
        ...
        try {
            ...
            await safeReply(interaction, { content: "❌ Erreur lors de la suppression.", ephemeral: true });
        }
    }

    // ── MODALS ──
    if (interaction.isModalSubmit()) {
        // Modal pseudo FiveM pour accepter un duel
        if (interaction.customId.startsWith("modal_accept_duel_")) {
            ...
            db.get("SELECT * FROM duels WHERE id = ? AND status = 'waiting'", [duelId], async (err, duel) => {
                if (!duel) return interaction.reply({ content: "❌ Ce duel n'existe plus ou a déjà commencé.", ephemeral: true });
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`La map a été tirée au sort : **${duel.map_name}**\n\nAllez faire votre meilleur temps sur cette map et cliquez **"J'ai fini"** quand c'est bon !`)
                    .addFields(
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith("accept_duel_")) {
            ...
            db.get("SELECT * FROM duels WHERE id = ?", [duelId], async (err, duel) => {
                ...
                // Ouvrir un modal pour demander le pseudo FiveM
                ...
                const input = new TextInputBuilder()
                    ...
                    .setLabel("Quel est ton pseudo exactement dans FiveM ?")
            ...
            db.get("SELECT * FROM duels WHERE id = ? AND status = 'ongoing'", [duelId], async (err, duel) => {
                ...
                if (newChallDone && newChalledDone) {
                    ...
                    if (!challengerTime && !challengedTime) {
                        resultText = "❌ Aucun temps trouvé pour les deux joueurs sur cette map !";
                    } else if (!challengerTime) {
                        winner = duel.challenged_id;
                        resultText = `Aucun temps trouvé pour **${duel.challenger_ig}** — victoire de **${duel.challenged_ig}** par forfait !`;
                    } else if (!challengedTime) {
                        winner = duel.challenger_id;
                        resultText = `Aucun temps trouvé pour **${duel.challenged_ig}** — victoire de **${duel.challenger_ig}** par forfait !`;
                    ...
                    if (winner) {
                        duelChannel.send(`🏆 GG <@${winner}> ! Tu remportes le duel sur **${duel.map_name}** ! 🔥`);
                    }
                }
            });
        }
    ...
    if (interaction.isStringSelectMenu()) {
        if (interaction.customId.startsWith("select_vote_")) {
            ...
            db.get("SELECT * FROM votes_sondage WHERE message_id = ? AND user_id = ?", [messageId, userId], async (err, row) => {
                if (row && !interaction.member.roles.cache.has(staffRoleId)) {
                    return interaction.editReply({ content: "⚠️ Tu as déjà voté ! Tu ne peux voter qu'une seule fois." });
                }
                db.run(
                    ...
                    async (err) => {
                        if (err) return interaction.editReply({ content: "❌ Erreur lors de l'enregistrement du vote." });
                        db.get(
                            ...
                            [messageId],
                            async (err, row) => {
                                if (err) return interaction.editReply({ content: "❌ Erreur lors de la mise à jour." });
                                ...
                                try {
                                    ...
                                    db.get("SELECT question FROM sondages_questions WHERE message_id = ?", [messageId], async (err, row) => {
                                        ...
                                        await interaction.editReply({ content: "✅ Votre vote a été enregistré !" });
                                    });
        ...
        db.get("SELECT * FROM votes WHERE vehicle_id = ? AND user_id = ?", [vehicleId, userId], async (err, row) => {
            if (row && !interaction.member.roles.cache.has(staffRoleId)) {
                return interaction.editReply({ content: "⚠️ Tu as déjà noté ce véhicule/parcours !" });
            }
...
client.on("messageCreate", async message => {
    ...
    if (message.content.startsWith("!duel") && message.channel.id === DUEL_CHANNEL_ID) {
        ...
        // !duel @pseudo MonPseudoFiveM
        if (args.length < 3) {
            return message.reply("❌ Utilisation : `!duel @joueur TonPseudoFiveM`");
        }
        ...
        const challengerIg = args[2];

        // Vérifier qu'aucun duel en cours
        db.get("SELECT * FROM duels WHERE (challenger_id = ? OR challenged_id = ?) AND status IN ('waiting', 'ongoing')",
            [message.author.id, message.author.id],
            async (err, existing) => {
                if (existing) return message.reply("❌ Tu as déjà un duel en cours !");

                // Tirer une map au sort
                ...
                db.run(
                    ...
                    function(err) {
                        ...
                        // Expirer après 5 minutes si pas accepté
                        setTimeout(() => {
