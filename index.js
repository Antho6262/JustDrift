...
// Maps drift disponibles pour les duels
const DRIFT_MAPS = [
    "Drifschool", "Speedway", "Observatoire", "Ebisu Kita", "Maple Valley",
    "Drift Park", "Petroll Hill", "Kami Road", "Nurb GP", "NordSchleife",
    "Downhill", "East Valley", "Nfs Carbon", "Usui", "Canyon"
];

// Premier tour = charger les records sans annoncer
let isFirstRun = true;
...
// Intercepter toutes les erreurs non gérées — le bot ne crash plus jamais
                    ...
                    if (bestAuthor) {
                        db.get("SELECT last_record FROM last_records WHERE race_name = ?", [raceName], (err, row) => {
                            ...
                            if (!row || row.last_record !== recordKey) {
                                ...
                                // Annoncer seulement si pas premier tour ET joueur différent
    ...
    try {
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId.startsWith("modal_accept_duel_")) {
                ...
                await interaction.reply({ content: `✅ Duel accepté ! Ton pseudo FiveM **${challengedIg}** a été enregistré. Rendez-vous sur la map !`, ephemeral: true }).catch(() => {});
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.startsWith("refuse_duel_")) {
                ...
                db.get("SELECT * FROM duels WHERE id = ?", [duelId], async (err, duel) => {
                    if (!duel) return;
                    if (interaction.user.id !== duel.challenged_id) return;
    ...
    if (message.content.startsWith("!duel") && message.channel.id === DUEL_CHANNEL_ID) {
        ...
        db.get("SELECT * FROM duels WHERE (challenger_id = ? OR challenged_id = ?) AND status IN ('waiting', 'ongoing')",
            ...
            async (err, existing) => {
                ...
                // Map aléatoire parmi les maps drift
