...
// Premier tour = on charge les records sans annoncer
let isFirstRun = true;
...
client.once("ready", async () => {
    ...
    // Vérifier les records toutes les 30 secondes
    const checkRecords = () => {
        fivemDb.query("SELECT NAME, DATA FROM aracing_leaderboards", (err, results) => {
                    ...
                    if (bestAuthor) {
                        db.get("SELECT last_record FROM last_records WHERE race_name = ?", [raceName], (err, row) => {
                            ...
                            if (!row || row.last_record !== recordKey) {
                                ...
                                // Ne pas annoncer au premier tour — juste mettre à jour la BDD
            ...
            // Après le premier tour, on active les annonces
            if (isFirstRun) {
                isFirstRun = false;
    ...
    // Premier appel immédiat pour charger les records sans annoncer
    checkRecords();
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.isButton()) {
        ...
        // ── NOTATION VOITURES/MAPS — ignorer si pas rate_ ou ratemap_ ──
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        // Ignorer les menus qui ne sont pas de notation
...
client.on("messageCreate", async message => {
    ...
    if (message.content.startsWith("!duel") && message.channel.id === DUEL_CHANNEL_ID) {
        ...
        // Supporte les pseudos FiveM avec espaces
        ...
        db.get(
            ...
            async (err, duel) => {
                ...
                if (!isStaff && message.author.id !== duel.challenger_id) {
                    return message.reply("❌ Seul le joueur qui a lancé le duel peut l'annuler. Le staff peut utiliser `!cancelduel @joueur`.");
                }
