...
// Message de bienvenue stylé en embed + attribution du rôle @Membre
client.on("guildMemberAdd", async member => {
    ...
    // Embed de bienvenue dans #🏎️・arrivants
    const welcomeChannelId = "1474139529786032201";
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.isCommand() && interaction.commandName === "clean") {
        ...
        try {
            ...
            await safeReply(interaction, { content: "❌ Erreur lors de la suppression des messages.", ephemeral: true });
        }
    }
    ...
    if (interaction.isStringSelectMenu()) {
            ...
            db.get("SELECT * FROM votes_sondage WHERE message_id = ? AND user_id = ?", [messageId, userId], async (err, row) => {
                ...
                db.run(
                    ...
                    async (err) => {
                        if (err) {
                            console.error(err);
                            return interaction.editReply({ content: "❌ Erreur lors de l'enregistrement du vote." });
                        }
                        ...
                        db.get(
                            ...
                            async (err, row) => {
                                if (err) {
                                    console.error(err);
                                    return interaction.editReply({ content: "❌ Erreur lors de la mise à jour." });
                                }
                                ...
                                try {
                                    ...
                                    db.get("SELECT question FROM sondages_questions WHERE message_id = ?", [messageId], async (err, row) => {
                                        if (err) {
                                            console.error(err);
                                            return interaction.editReply({ content: "❌ Erreur lors de la récupération de la question." });
                                        }
                                    ...
                                    console.error(error);
                                    await interaction.editReply({ content: "❌ Erreur lors de la mise à jour du message." });
                                }
                            }
                        );
                    }
        ...
        db.get("SELECT * FROM votes WHERE vehicle_id = ? AND user_id = ?", [vehicleId, userId], async (err, row) => {
            if (row && !interaction.member.roles.cache.has(staffRoleId)) {
                return interaction.editReply({ content: "⚠️ Tu as déjà noté ce véhicule/parcours ! Tu ne peux voter qu'une seule fois." });
            }
...
client.on("messageCreate", async message => {
    ...
    if (message.content.startsWith("!addcar")) {
        ...
        if (!nom) return message.reply("❌ Utilisation : `!addcar Nom du véhicule` avec une image en pièce jointe.");
        if (message.attachments.size === 0) return message.reply("❌ Tu dois joindre une image de la voiture !");

        const image = message.attachments.first();

        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`rate_PLACEHOLDER`).setLabel("Noter ce véhicule").setStyle(ButtonStyle.Primary)
        );
        ...
        const updatedRow = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`rate_${carMessage.id}`).setLabel("Noter ce véhicule").setStyle(ButtonStyle.Primary)
        );
    ...
    if (message.content.startsWith("!addmap")) {
        ...
        if (!nom) return message.reply("❌ Utilisation : `!addmap Nom du parcours` avec une image en pièce jointe.");
        if (message.attachments.size === 0) return message.reply("❌ Tu dois joindre une image du parcours !");

        const image = message.attachments.first();

        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`ratemap_PLACEHOLDER`).setLabel("Noter ce parcours").setStyle(ButtonStyle.Success)
        );
        ...
        const updatedRow = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`ratemap_${mapMessage.id}`).setLabel("Noter ce parcours").setStyle(ButtonStyle.Success)
        );
    ...
    if (message.content.startsWith("!addvote")) {
        ...
        if (!question) return message.reply("❌ Utilisation : `!addvote Votre question ici`");

        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`vote_button_PLACEHOLDER`).setLabel("Votez").setStyle(ButtonStyle.Primary)
        );
        ...
        const updatedRow = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(`vote_button_${voteMessage.id}`).setLabel("Votez").setStyle(ButtonStyle.Primary)
        );
    ...
    if (message.content === "!topcar") {
        db.all(
            ...
            async (err, rows) => {
                if (err) { console.error(err); return message.reply("❌ Erreur lors de la récupération du classement."); }
                if (!rows || rows.length === 0) return message.reply("❌ Aucun véhicule noté pour le moment.");
    ...
    if (message.content === "!topmap") {
        db.all(
            ...
            async (err, rows) => {
                if (err) { console.error(err); return message.reply("❌ Erreur lors de la récupération du classement."); }
                if (!rows || rows.length === 0) return message.reply("❌ Aucune map notée pour le moment.");
