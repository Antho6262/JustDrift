...
fivemDb.connect((err) => {
    if (err) { console.error("❌ Erreur connexion BDD FiveM:", err); }
    else { console.log("✅ Connecté à la BDD FiveM"); }
});
...
async function safeReply(interaction, options) {
    ...
    } catch (err) { console.error("Erreur safeReply:", err); }
}

function getAllMaps() {
    return new Promise((resolve) => {
        fivemDb.query("SELECT NAME FROM aracing_leaderboards", (err, results) => {
...
function getPlayerBestTime(playerName, mapName) {
    return new Promise((resolve) => {
        fivemDb.query("SELECT DATA FROM aracing_leaderboards WHERE NAME = ?", [mapName], (err, results) => {
            ...
            try {
                ...
                for (const [carHash, records] of Object.entries(data)) {
                    if (Array.isArray(records)) {
                        ...
                        if (playerRecord && (!bestTime || playerRecord.Record < bestTime)) {
                            bestTime = playerRecord.Record;
                        }
                    }
                }
                resolve(bestTime);
...
client.once("ready", async () => {
    ...
    if (guild) {
        ...
        } catch (error) { console.error("Erreur enregistrement commande:", error); }
    }
        ...
        fivemDb.query("SELECT NAME, DATA FROM aracing_leaderboards", (err, results) => {
            ...
            results.forEach(race => {
                ...
                try {
                    ...
                    let bestAuthor = null, bestTime = null;
                    for (const [carHash, records] of Object.entries(data)) {
                        ...
                        db.get("SELECT last_record FROM last_records WHERE race_name = ?", [raceName], (err, row) => {
                            ...
                            if (!row || row.last_record !== recordKey) {
                                ...
                                if (previousHolder !== bestAuthor) {
                                    ...
                                    if (recordChannel) {
                                        if (previousHolder) {
                                            recordChannel.send(`🏆 **${bestAuthor}** a battu le record de **${previousHolder}** sur **${raceName}** !`);
                                        } else {
                                            recordChannel.send(`🏆 **${bestAuthor}** a établi le premier record sur **${raceName}** !`);
                                        }
                                    }
                                }
                                db.run(
                ...
                } catch (e) { console.error(`Erreur parsing JSON pour ${raceName}:`, e); }
            });
        });
    }, 30000);
...
client.on("guildMemberAdd", async member => {
    ...
    if (role) {
        ...
        catch (error) { console.error(`Erreur rôle ${member.user.username}:`, error); }
    }
...
client.on("guildMemberRemove", async member => {
    ...
    catch (error) { console.error("Erreur message départ:", error); }
});

client.on("interactionCreate", async interaction => {
    if (interaction.isCommand() && interaction.commandName === "clean") {
        if (!interaction.member.roles.cache.has(staffRoleId)) {
            return interaction.reply({ content: "❌ Tu dois avoir le rôle **STAFF**.", ephemeral: true });
        }
        ...
        } catch { await safeReply(interaction, { content: "❌ Erreur lors de la suppression.", ephemeral: true }); }
    }
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.customId.startsWith("modal_accept_duel_")) {
            ...
            db.get("SELECT * FROM duels WHERE id = ? AND status = 'waiting'", [duelId], async (err, duel) => {
                if (!duel) return interaction.reply({ content: "❌ Ce duel n'existe plus.", ephemeral: true });
                if (interaction.user.id !== duel.challenged_id) return interaction.reply({ content: "❌ Ce duel ne te concerne pas.", ephemeral: true });
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`La map a été tirée au sort : **${duel.map_name}**\n\nAllez faire votre meilleur temps et cliquez **"J'ai fini"** quand c'est bon !`)
                    .addFields(
                ...
                await interaction.reply({ content: `✅ Duel accepté ! Rendez-vous sur **${duel.map_name}** !`, ephemeral: true });
            });
        }
    }
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith("refuse_duel_")) {
            ...
            db.get("SELECT * FROM duels WHERE id = ?", [duelId], async (err, duel) => {
                ...
                await interaction.reply({ content: "✅ Duel refusé.", ephemeral: true });
            });
        }
        ...
        if (interaction.customId.startsWith("duel_done_")) {
            ...
            db.get("SELECT * FROM duels WHERE id = ? AND status = 'ongoing'", [duelId], async (err, duel) => {
                if (!duel) return interaction.reply({ content: "❌ Duel introuvable ou déjà terminé.", ephemeral: true });

                const isChallenger = interaction.user.id === duel.challenger_id;
                const isChallenged = interaction.user.id === duel.challenged_id;
                if (!isChallenger && !isChallenged) return interaction.reply({ content: "❌ Tu ne participes pas à ce duel.", ephemeral: true });
                if (isChallenger && duel.challenger_done) return interaction.reply({ content: "⚠️ Tu as déjà indiqué que tu avais fini !", ephemeral: true });
                if (isChallenged && duel.challenged_done) return interaction.reply({ content: "⚠️ Tu as déjà indiqué que tu avais fini !", ephemeral: true });
                ...
                await interaction.reply({ content: "✅ Noté ! En attente de l'adversaire...", ephemeral: true });
                ...
                if (newChallDone && newChalledDone) {
                    ...
                    const [challengerTime, challengedTime] = await Promise.all([
                        getPlayerBestTime(duel.challenger_ig, duel.map_name),
                        getPlayerBestTime(duel.challenged_ig, duel.map_name)
                    ]);
                    ...
                    if (!challengerTime && !challengedTime) {
                        resultText = "❌ Aucun temps trouvé pour les deux joueurs !";
                    } else if (!challengerTime) {
                        winner = duel.challenged_id;
                        resultText = `Aucun temps pour **${duel.challenger_ig}** — victoire de **${duel.challenged_ig}** par forfait !`;
                    } else if (!challengedTime) {
                        winner = duel.challenger_id;
                        resultText = `Aucun temps pour **${duel.challenged_ig}** — victoire de **${duel.challenger_ig}** par forfait !`;
                    } else if (challengerTime < challengedTime) {
                        winner = duel.challenger_id;
                    ...
                    const resultEmbed = new EmbedBuilder()
                        ...
                        .addFields(
                            { name: `🏎️ ${duel.challenger_ig}`, value: `⏱️ ${formatTime(challengerTime)}`, inline: true },
                            { name: "⚔️ VS", value: "\u200b", inline: true },
                            { name: `🏎️ ${duel.challenged_ig}`, value: `⏱️ ${formatTime(challengedTime)}`, inline: true }
                        )
                        .setFooter({ text: "JustDrift • Système de duels" })
                        .setTimestamp();

                    if (msg) await msg.edit({ embeds: [resultEmbed], components: [] });
                    if (winner) duelChannel.send(`🏆 GG <@${winner}> ! Tu remportes le duel sur **${duel.map_name}** ! 🔥`);
                }
            });
        }
        ...
        if (interaction.customId.startsWith("close_ticket_")) {
            ...
            if (!ticketChannel) return interaction.reply({ content: "❌ Salon introuvable", ephemeral: true });
    ...
    if (interaction.isStringSelectMenu()) {
        if (interaction.customId.startsWith("select_vote_")) {
            ...
            db.get("SELECT * FROM votes_sondage WHERE message_id = ? AND user_id = ?", [messageId, userId], async (err, row) => {
                if (row && !interaction.member.roles.cache.has(staffRoleId)) return interaction.editReply({ content: "⚠️ Tu as déjà voté !" });
                db.run(
                    ...
                    async (err) => {
                        if (err) return interaction.editReply({ content: "❌ Erreur enregistrement." });
                        db.get(
                            ...
                            [messageId],
                            async (err, row) => {
                                if (err) return interaction.editReply({ content: "❌ Erreur mise à jour." });
                                ...
                                try {
                                    ...
                                    db.get("SELECT question FROM sondages_questions WHERE message_id = ?", [messageId], async (err, row) => {
                                        if (err) return interaction.editReply({ content: "❌ Erreur question." });
                                        const question = row ? row.question : "Sondage";
                                        await msg.edit({ content: `📊 **Sondage**\n${question}\n\n✅ Oui : ${oui} votes\n❌ Non : ${non} votes\n❓ Ne sais pas : ${neSaisPas} votes\n\nTotal : ${total} votes` });
                                        await interaction.editReply({ content: "✅ Vote enregistré !" });
                                    });
                                } catch { await interaction.editReply({ content: "❌ Erreur mise à jour message." }); }
                            }
                        );
                    }
                );
        ...
        db.get("SELECT * FROM votes WHERE vehicle_id = ? AND user_id = ?", [vehicleId, userId], async (err, row) => {
            if (row && !interaction.member.roles.cache.has(staffRoleId)) return interaction.editReply({ content: "⚠️ Tu as déjà noté ce véhicule/parcours !" });
            ...
            db.get("SELECT AVG(note) AS avg, COUNT(*) AS total FROM votes WHERE vehicle_id = ?", [vehicleId], async (err, row) => {
                ...
                try {
                    ...
                    await interaction.editReply({ content: "✅ Ta note a été enregistrée !" });
                } catch { await interaction.editReply({ content: "❌ Erreur mise à jour." }); }
            });
        });
    }
});
...
client.on("messageCreate", async message => {
    ...
    if (message.content.startsWith("!duel") && message.channel.id === DUEL_CHANNEL_ID) {
        ...
        // Tout ce qui est après la mention = pseudo FiveM (supporte les espaces)
        ...
        db.get("SELECT * FROM duels WHERE (challenger_id = ? OR challenged_id = ?) AND status IN ('waiting', 'ongoing')",
            ...
            async (err, existing) => {
                ...
                if (!maps.length) return message.reply("❌ Aucune map disponible dans la BDD !");
                ...
                db.run(
                    ...
                    function(err) {
                        ...
                        duelMsg.edit({ embeds: [embed], components: [row] });
    ...
    // ── COMMANDE !cancelduel ──
    if (message.content === "!cancelduel" && message.channel.id === DUEL_CHANNEL_ID) {
        const isStaff = message.member.roles.cache.has(staffRoleId);

        // Staff peut annuler n'importe quel duel en mentionnant un joueur
        // Sinon on annule son propre duel
        ...
        db.get(
            ...
            async (err, duel) => {
                if (!duel) {
                    return message.reply("❌ Aucun duel en cours trouvé.");
                }

                // Si pas staff, seul le challenger peut annuler son propre duel
                if (!isStaff && message.author.id !== duel.challenger_id) {
                    return message.reply("❌ Seul le joueur qui a lancé le duel peut l'annuler. Le staff peut aussi le faire avec `!cancelduel @joueur`.");
                }
                ...
                const duelChannel = client.channels.cache.get(DUEL_CHANNEL_ID);
                const msg = duelChannel ? await duelChannel.messages.fetch(duel.message_id).catch(() => null) : null;
                if (msg) await msg.edit({ embeds: [cancelEmbed], components: [] });

                await message.reply("✅ Duel annulé !");
                await message.delete().catch(() => {});
            }
        );
        return;
